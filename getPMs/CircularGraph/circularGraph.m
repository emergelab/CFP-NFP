...
% Copyright 2016 The MathWorks, Inc.
  ...
  methods
    function this = circularGraph(adjacencyMatrix,varargin)
      ...
      addParameter(p,'StartPoint'   ,defaultStartPoint   ,@isnumeric);
      
      parse(p,adjacencyMatrix,varargin{:});
      this.ColorMap = p.Results.ColorMap;
      this.Label    = p.Results.Label;
      this.LineWidth    = p.Results.LineWidth;
      this.StartPoint    = p.Results.StartPoint;
      ...
      % Draw the nodes
      delete(this.Node);
      t = linspace(this.StartPoint*pi,(2+this.StartPoint)*pi,length(adjacencyMatrix) + 1).'; % theta for each node
      ...
      for i = 1:length(v)
        if row(i) ~= col(i)
          if abs(row(i) - col(i)) - length(adjacencyMatrix)/2 == 0 
            ...
            % points are diametric, so draw a straight line
            u = [cos(t(row(i)));sin(t(row(i)))];
            this.Node(row(i)).Connection(end+1) = viscircles([u(1)...
            ,u(2)],0.05,'LineWidth', lineWidth(i),'Color',this.ColorMap(row(i),:));
        end
      end
